...
class PolicyRepository(LoggerMixin):
    ...
    def create_policy(self, policy_data: PolicyCreate) -> Policy:
        ...
        policy = Policy(
            policy_id=policy_id, created_at=now, updated_at=now, **policy_data.dict()
        )
        ...
        try:
            ...
            self.dynamodb.put_item(
                ...
                Item={
                    ...
                    "TTL": {
                        "N": str(int(now.timestamp() + 86400 * 365 * 10))
                    },  # 10 years
                },
            )
            ...
            self.logger.error(
                "Failed to create policy", error=str(e), policy_id=policy_id
            )
    ...
    def get_customer_policies(
        ...
        try:
            ...
            if next_token:
                query_params["ExclusiveStartKey"] = eval(
                    next_token
            ...
            self.logger.error(
                "Failed to get customer policies", error=str(e), customer_id=customer_id
            )
            raise RuntimeError(f"Failed to get customer policies: {str(e)}") from e

    def update_policy(
        self, policy_id: str, update_data: PolicyUpdate
    ) -> Policy | None:
        """Update a policy."""
        ...
        try:
            ...
            expression_values = {
                ":updated_at": {"S": updated_policy.updated_at.isoformat()}
            }
            ...
            self.logger.error(
                "Failed to update policy", error=str(e), policy_id=policy_id
            )
