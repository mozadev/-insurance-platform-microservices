    ...
    def search(
        ...
        try:
            return self.repository.search(
                query=query, page=page, size=size, filters=filters, sort=sort
            )
...
async def search(
    ...
    sort: str | None = Query(
        None, description="Sort field (prefix with - for desc)"
    ),
    ...
    return service.search(
        query=q, page=page, size=size, filters=filters if filters else None, sort=sort
    )
