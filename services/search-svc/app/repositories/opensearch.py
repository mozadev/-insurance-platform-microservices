...
from ..models import IndexDocument, SearchResponse, SearchResult
...
class SearchRepository(LoggerMixin):
    ...
    def search(
        ...
        try:
            ...
            response = self.client.search(
                index=f"{self.claims_index},{self.policies_index}", body=search_body
            )
    ...
    def index_document(self, document: IndexDocument) -> bool:
        ...
        try:
            index_name = (
                self.claims_index if document.type == "claim" else self.policies_index
            )
            ...
            response = self.client.index(
                index=index_name, id=document.id, body=doc_body
            )
    ...
    def bulk_index(self, documents: list[IndexDocument]) -> tuple[int, int]:
        ...
        try:
            ...
            for doc in documents:
                index_name = (
                    self.claims_index if doc.type == "claim" else self.policies_index
                )
                ...
                action = {"_index": index_name, "_id": doc.id, "_source": doc_body}
    ...
    def delete_document(self, doc_id: str, doc_type: str) -> bool:
        ...
        try:
            index_name = (
                self.claims_index if doc_type == "claim" else self.policies_index
            )
    ...
    def get_document(self, doc_id: str, doc_type: str) -> dict[str, Any] | None:
