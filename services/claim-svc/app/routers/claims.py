...
class ClaimService(LoggerMixin):
    ...
    def create_claim(self, claim_data: ClaimCreate, customer_id: str) -> ClaimResponse:
        ...
        if claim_data.idempotency_key:
            ...
            cached_response = self.idempotency_manager.get_response(
                claim_data.idempotency_key, request_data
            )
            if cached_response:
                self.logger.info(
        ...
        if claim_data.idempotency_key:
            ...
            self.idempotency_manager.put_if_absent(
                claim_data.idempotency_key, request_data, response_data
            )
    ...
    def get_policy_claims(
        ...
        customer_claims = [
            claim for claim in claims if claim.customer_id == customer_id
        ]

        claim_responses = [ClaimResponse(**claim.dict()) for claim in customer_claims]

        return ClaimListResponse(
            claims=claim_responses, total=len(claim_responses), next_token=next_token
        )

    def update_claim(
        self, claim_id: str, update_data: ClaimUpdate, customer_id: str
    ) -> ClaimResponse:
        """Update a claim."""
