...
class ClaimRepository(LoggerMixin):
        ...
        try:
            ...
            self.dynamodb.put_item(
                ...
                Item={
                    ...
                    "TTL": {
                        "N": str(int(now.timestamp() + 86400 * 365 * 7))
                    },  # 7 years
                },
            )
    ...
    def get_policy_claims(
        self, policy_id: str, limit: int = 20, next_token: str | None = None
        ...
        try:
            query_params = {
                ...
                "ExpressionAttributeValues": {":policy_id": {"S": f"POL#{policy_id}"}},
    ...
    def update_claim(self, claim_id: str, update_data: ClaimUpdate) -> Claim | None:
        ...
        try:
            ...
            expression_values = {
                ":updated_at": {"S": updated_claim.updated_at.isoformat()}
            }
