    ...
    def put_if_absent(
        ...
        try:
            ...
            if e.response['Error']['Code'] == 'ConditionalCheckFailedException':
                ...
                self.logger.error("Failed to store idempotency key", error=str(e), key=key)
                raise RuntimeError(f"Failed to store idempotency key: {str(e)}")
    
    def get_response(self, idempotency_key: str, request_data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            self.logger.error("Failed to get idempotency response", error=str(e), key=key)
            return None
