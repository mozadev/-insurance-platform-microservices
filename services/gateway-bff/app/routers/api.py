...
class GatewayService(LoggerMixin):
    ...
    async def get_customer_dashboard(self, customer_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Get recent claims
            claims_response = await self.claim_client._make_request_with_retry(
                "GET", f"/claims/policy/{policies_response['policies'][0]['policyId']}"
            ) if policies_response['policies'] else {"claims": []}
            ...
            return {
                ...
                "summary": {
                    ...
                    "active_policies": len([p for p in policies_response.get("policies", []) if p.get("status") == "active"]),
                    "total_claims": len(claims_response.get("claims", [])),
                    "open_claims": len([c for c in claims_response.get("claims", []) if c.get("status") == "open"])
                }
            }
        except Exception as e:
            self.logger.error("Failed to get customer dashboard", error=str(e), customer_id=customer_id)
            raise HTTPException(
