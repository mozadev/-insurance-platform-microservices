...
def mock_dynamodb():
    ...
    with patch(
        "services.claim_svc.app.repositories.dynamodb.get_dynamodb_client"
    ) as mock:
        mock_client = Mock()
...
def test_create_claim(client, mock_dynamodb, mock_sns):
    ...
    with patch(
        "services.claim_svc.app.routers.claims.get_current_customer"
    ) as mock_auth:
...
def test_idempotency(client, mock_dynamodb, mock_sns):
    ...
    with patch(
        "services.claim_svc.app.idempotency.dynamodb.IdempotencyManager.get_response"
    ) as mock_idem:
