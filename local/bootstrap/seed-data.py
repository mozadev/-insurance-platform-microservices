    ...
    for policy in policies:
        aws_clients['dynamodb'].put_item(
            ...
            Item={
                ...
                'GSI1SK': {'S': f"POL#{policy['policyId']}#{int(datetime.utcnow().timestamp())}"},
                'TTL': {'N': str(int((datetime.utcnow().timestamp() + 86400 * 365 * 10)))},
            }
        )
    ...
    for claim in claims:
        aws_clients['dynamodb'].put_item(
            ...
            Item={
                ...
                'GSI1SK': {'S': f"STATUS#{claim['status']}#{int(datetime.utcnow().timestamp())}"},
                'TTL': {'N': str(int((datetime.utcnow().timestamp() + 86400 * 365 * 7)))},
            }
        )
...
def publish_events(aws_clients: Dict[str, Any], policies: list[Dict[str, Any]], claims: list[Dict[str, Any]]):
    ...
    for claim in claims:
        ...
        aws_clients['sns'].publish(
            ...
            MessageAttributes={
                'eventType': {
                    'DataType': 'String',
                    'StringValue': 'ClaimCreated'
                }
            }
        )


def index_in_opensearch(opensearch_client: OpenSearch, policies: list[Dict[str, Any]], claims: list[Dict[str, Any]]):
